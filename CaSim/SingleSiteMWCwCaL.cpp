...
SingleSiteMWCwCaL::SingleSiteMWCwCaL(std::map<std::string, double>& params) {
	fCa4I = params["fCa4I"];
	fCa = params["fCa"];
	KCa = params["KCa"];
...
double SingleSiteMWCwCaL::_fCa(double CaL) {
	return f0 + df * std::pow(CaL, nCaL) / (std::pow(CaL, nCaL) + std::pow(KCaL, nCaL));
}

void SingleSiteMWCwCaL::_AddTwoStatesInfo(int x, int y) {
...
void SingleSiteMWCwCaL::_BuildAdjacencyList() {
	int idx_c, idx_c2;
	for (int i = 0; i < 5; ++i)
		for (int j = 0; j < 5 - i; ++j) {
...
void SingleSiteMWCwCaL::SetRow(int i, double ca, int j, double mg, double cal) {
	double fCa = _fCa(cal);
	...
	Q[idx_c][idx_c + 15] = konMgI * std::pow(mg, 2) / std::pow(fIC, alphaI);
	Q[idx_c + 15][idx_c] = koffMgI * std::pow(fIC, 1 - alphaI);
	Q[idx_c + 45][idx_c + 30] = konMgI * std::pow(mg, 2) / std::pow(fIO, alphaI);
	if (i == 4)
