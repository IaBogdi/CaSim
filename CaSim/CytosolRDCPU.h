...
class CytosolRDCPU : public CytosolSeries {
public:
	...
	inline double dYdX(double, double, double);
	int n_ions;
	int _Index(int, int);
	void _EvolutionOp(int, int, double);
	void _GetFreeBuffers(int, int);
	void _UpdateEvo(int, int, double);
	void _GiveIonsAndBuffers();
public:
	CytosolRDCPU(nlohmann::json&, nlohmann::json&, int);
	CytosolRDCPU(nlohmann::json&, nlohmann::json&, nlohmann::json&, int);
	bool UsesGPU();
	void Reset();
	~CytosolRDCPU();
