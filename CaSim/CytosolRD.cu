...
__device__ __constant__ double _dr;
__device__ __constant__ double _R;
...
cudaStream_t stream_cyto;
cudaGraph_t timestep_cyto_graph;
cudaGraphExec_t timestep_cyto_instance;

__device__ double d2YdX2_cyto(double Yl, double Yc, double Yr, double dX) {
	return (Yr - 2 * Yc + Yl) / (dX * dX);
}

__device__ double dYdX_cyto(double Yl, double Yr, double dX) {
	return (Yr - Yl) / (2 * dX);
}

__global__ void set_initials_cyto(double* ions, double* init_ions, double* buffers, double* init_buffers, double* buf_free, double* buf_tot, short* ion_buffer_table) {
...
__global__ void diffusion_step_cyto(double* ions, double* buffers, double* evo_ions, double* evo_buffers, double* D_ions, double* D_buf, short* ion_buffer_table) {
	...
	if (idr_rec < _nr - 1 && idx_sim < _n_threads) {
		if (idr_rec > 0) {
			...
			int idrr = idfull + 1;
			double val_center;
			double r = _dr * idr_rec;
			int str;
			for (int j = 0; j < _n_ions; ++j) {
				str = j * _n_elements;
				val_center = ions[idfull + str];
				evo_ions[idfull + str] = D_ions[j] * (2 * dYdX_cyto(ions[idrl + str], ions[idrr + str], _dr) / r +  d2YdX2_cyto(ions[idrl + str], val_center, ions[idrr + str], _dr));
				for (int i = 0; i < _n_buffers; ++i) {
					str = i * _n_elements;
					val_center = buffers[idfull + str];
					evo_buffers[idfull + str] = D_buf[i] * (2 * dYdX_cyto(buffers[idrl + str], buffers[idrr + str], _dr) / r + d2YdX2_cyto(buffers[idrl + str], val_center, buffers[idrr + str], _dr));
				}
			}
		}
		else {
			...
			for (int j = 0; j < _n_ions; ++j) {
				str = j * _n_elements;
				val_center = ions[idfull + str];
				evo_ions[idfull + str] = D_ions[j] * (d2YdX2_cyto(ions[idrr + str], val_center, ions[idrr + str], _dr));
				for (int i = 0; i < _n_buffers; ++i) {
					str = i * _n_elements;
					val_center = buffers[idfull + str];
					evo_buffers[idfull + str] = D_buf[i] * (d2YdX2_cyto(buffers[idrr + str], val_center, buffers[idrr + str], _dr));
				}
			}
		}
	}
...
__global__ void reaction_step_cyto(double* ions, double* buffers, double* evo_ions, double* evo_buffers, double* kon, double* koff, double* buf_free, short* ion_buffer_table, short* is_in_dyad) {
	int idr_rec = threadIdx.x + blockDim.x * blockIdx.x; //2D - (r,n_thread)
	int idx_sim = threadIdx.y + blockDim.y * blockIdx.y;
	int idfull = idr_rec + _nr * idx_sim;
	if (idr_rec < _nr - 1 && idx_sim < _n_threads) {
...
__global__ void sum_step_cyto(double* evo_ions_diffusion, double* ions, double* evo_buffers_diffusion, double* evo_ions_reaction, double* evo_buffers_reaction, double* currents, double* evo_ions_total, double* evo_buffers_total, double* Jmax, double* K, double* ionsb, short* ion_buffer_table, short* ion_SERCA_table, short* is_in_dyad) {
	int idr_rec = threadIdx.x + blockDim.x * blockIdx.x; //2D - (r,n_thread)
	int idx_sim = threadIdx.y + blockDim.y * blockIdx.y;
	int idfull = idr_rec + _nr * idx_sim;
	int idx_cur = 0;
	if (idr_rec < _nr - 1 && idx_sim < _n_threads) {
			...
			for (int j = 0; j < _n_ions; ++j) {
				...
				if (ion_SERCA_table[j]) {
					...
					double K2 = K[_idx_SERCA] * K[_idx_SERCA];
					double ions2b = ionsb[_idx_SERCA] * ionsb[_idx_SERCA];
...
__global__ void evo_step_cyto(double* evo_ions_total, double* ions, double* ions_f, double* evo_buffers_total, double* buffers, double* buffers_f, short* ion_buffer_table) {
	int idr_rec = threadIdx.x + blockDim.x * blockIdx.x; //2D - (r,n_thread)
	int idx_sim = threadIdx.y + blockDim.y * blockIdx.y;
	int idfull = idr_rec + _nr * idx_sim;
	if (idr_rec < _nr - 1 && idx_sim < _n_threads) {
		...
		for (int j = 0; j < _n_ions; ++j) {
			idx2 = idfull + j * _n_elements;
			ions[idx2] = ions_f[idx2]  + evo_ions_total[idx2] * _dt;
			ions_f[idx2] = ions[idx2];
		}
		for (int i = 0; i < _n_buffers; ++i) {
			idx2 = idfull + i * _n_elements;
			buffers[idx2] = buffers_f[idx2] + evo_buffers_total[idx2] * _dt;
			buffers_f[idx2] = buffers[idx2];
		}
	}
}

__global__ void get_free_buffers_cyto(double* buffers, double* buf_free, double* buf_tot, short* ion_buffer_table) {
	int idr_rec = threadIdx.x + blockDim.x * blockIdx.x; //2D - (r,n_thread)
	int idx_sim = threadIdx.y + blockDim.y * blockIdx.y;
	int idfull = idr_rec + _nr * idx_sim;
	if (idr_rec < _nr - 1 && idx_sim < _n_threads) {
...
__global__ void give_ions_and_buffers(double* ions_boundary, double* buffers_boundary, double* ions, double* buffers, short* is_in_dyad) {
...
void CytosolRD::Initialize(nlohmann::json& j, nlohmann::json& j_dyad, int nthreads) {
	dr = j["dr"];
	...
	for (auto const& el : j_dyad["Buffers"].items()) {
		...
		buffers.push_back(std::make_unique<Buffer<double> >(el.key(), pars_buff["Total Concentration"], true));
	...
	cudaMalloc(&d_ionsb, ions_bg.size() * sizeof(double));
	cudaMemcpy(d_ionsb, ions_bg.data(), ions_bg.size() * sizeof(double), cudaMemcpyHostToDevice);
	...
	cudaMalloc(&d_buffers_free, n_elements * idx_buf * sizeof(double));
	...
	cudaMalloc(&d_Jmax, Jmax.size() * sizeof(double));
	cudaMemcpy(d_Jmax, Jmax.data(), Jmax.size() * sizeof(double), cudaMemcpyHostToDevice);
	cudaMalloc(&d_Kup, Kup.size() * sizeof(double));
	cudaMemcpy(d_Kup, Kup.data(), Kup.size() * sizeof(double), cudaMemcpyHostToDevice);
}

CytosolRD::CytosolRD(nlohmann::json& j, nlohmann::json& j_dyad, int nthreads) {
	Initialize(j, j_dyad, nthreads);
}

std::vector<double> CytosolRD::GaussElimination(std::vector<std::vector<double> >& a) {
	int n = (int)a.size();
...
CytosolRD::CytosolRD(nlohmann::json& j, nlohmann::json& j_dyad, nlohmann::json& j_nsr, int nthreads) {
...
void CytosolRD::_SetNumIonsandBuffsinDyad(int nions, int nbuf) {
	n_ions_dyad = nions;
...
std::vector<double>& CytosolRD::GetExtraCellularIons() {
	return extracell_ions;
}


void CytosolRD::_SetupStepGraph() {
	cudaStreamBeginCapture(stream_cyto, cudaStreamCaptureModeGlobal);
	diffusion_step_cyto << <grid, threads_per_block, 0, stream_cyto >> > (d_ions_f, d_buffers_f, evo_ions_diffusion, evo_buffers_diffusion, d_D_ions, d_D_buf, d_buffer_binds_ion);
	reaction_step_cyto << <grid, threads_per_block, 0, stream_cyto >> > (d_ions_f, d_buffers_f, evo_ions_reaction, evo_buffers_reaction, d_kon, d_koff, d_buffers_free, d_buffer_binds_ion, d_is_in_dyad);
	sum_step_cyto << <grid, threads_per_block, 0, stream_cyto >> > (evo_ions_diffusion, d_ions_f, evo_buffers_diffusion, evo_ions_reaction, evo_buffers_reaction, d_currents, evo_ions_total, evo_buffers_total, d_Jmax, d_Kup, d_ionsb, d_buffer_binds_ion, d_SR_uptakes_ion,d_is_in_dyad);
	evo_step_cyto << <grid, threads_per_block, 0, stream_cyto >> > (evo_ions_total, d_ions, d_ions_f, evo_buffers_total, d_buffers, d_buffers_f, d_buffer_binds_ion); //DOUBLE PRECISION KERNEL 
	get_free_buffers_cyto << <grid, threads_per_block, 0, stream_cyto >> > (d_buffers_f, d_buffers_free, d_CTot, d_buffer_binds_ion);
	cudaStreamEndCapture(stream_cyto, &timestep_cyto_graph);
	cudaGraphInstantiate(&timestep_cyto_instance, timestep_cyto_graph, NULL, NULL, 0);
}


void CytosolRD::Update(double*& dyad_currents, const std::vector<double>& nsr_ions) {
...
void CytosolRD::RunRD(double dt, int idx_b) {
	if (!dt_is_set) {
		cudaMemcpyToSymbol(_dt, &dt, sizeof(double));
		dt_is_set = true;
	}
	cudaGraphLaunch(timestep_cyto_instance, stream_cyto);
}

void CytosolRD::GetIonsandBuffers(double*& d_ions_boundary_out, double*& d_buffers_boundary_out) {
	give_ions_and_buffers << <1, n_threads >> > (d_ions_boundary, d_buffers_boundary, d_ions_f, d_buffers_f, d_is_in_dyad);
	d_ions_boundary_out = d_ions_boundary;
	d_buffers_boundary_out = d_buffers_boundary;
}


bool CytosolRD::UsesGPU() {
	return true;
}

std::map < std::string, std::vector<double> > CytosolRD::GetConcentrations(std::vector<std::string>& values) {
	...
	for (int i = 0; i < ions.size(); ++i) {
		...
		if (it != values.end()) {
			...
			cudaMemcpyAsync(out[ions[i]->name].data(), d_ions_f + i * n_elements, n_elements * sizeof(double), cudaMemcpyDeviceToHost, stream_cyto);
		}
	}
		...
		if (it != values.end()) {
			...
			for (auto const& ion : ions) {
				...
				if (it2 != buffers[i]->ions_kinetics.end()) {
					...
					cudaMemcpyAsync(out[type_name].data(), d_buffers_f + idx_b * n_elements, n_elements * sizeof(double), cudaMemcpyDeviceToHost, stream_cyto);
				}
			}
		}
	}
...
void CytosolRD::Reset() {
	set_initials_cyto << <grid, block >> > (d_ions_f, d_init_ions, d_buffers_f, d_init_buffers, d_buffers_free, d_CTot, d_buffer_binds_ion);
	cudaMemcpy(d_currents, zero_vector.data(), zero_vector.size() * sizeof(double), cudaMemcpyHostToDevice);
}

std::vector<uint64_t> CytosolRD::GetDimensions() {
	return cytosol_dims;
}
...
CytosolRD::~CytosolRD() {
	...
	cudaGraphExecDestroy(timestep_cyto_instance);
	cudaGraphDestroy(timestep_cyto_graph);
	cudaStreamDestroy(stream_cyto);
}